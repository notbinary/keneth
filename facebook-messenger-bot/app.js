...
function receivedMessage(event) {
  ...
  if (messageText) {
    ...
    switch (messageText.replace(/[^\w\s]/gi, '').trim().toLowerCase()) {
      ...
      default:
        acknowledgeVehicleCheckRequest(senderID);

        var request = require('request');
        var FormData = require('form-data');
        var form = new FormData();
        form.append('vrm', messageText);
        checkVehicleAndRespond(form, senderID);
    }
    ...
    var FormData = require('form-data');
    var form = new FormData();
    form.append('photo', request(messageAttachments[0].payload.url));
    checkVehicleAndRespond(form, senderID);
  }
}

function acknowledgeVehicleCheckRequest(senderID) {
    sendReadReceipt(senderID);
    sendTypingOn(senderID);
}

function checkVehicleAndRespond(form, senderID) {
  form.submit('http://localhost:6000/json', function(error, response) {
    ...
    response.on('end', function() {
      console.log(body);
      var vehicleDetails = JSON.parse(body);
      sendTypingOff(senderID);
      sendVehicleDetails(senderID, vehicleDetails);
    });
...
function sendVehicleDetails(senderID, vehicleDetails) {
  sendTextMessage(
    senderID,
    "I found these vehicle details for you... \n\n" +
...
function sendHiMessage(recipientId) {
  var messageData = {
    ...
    message: {
      text: `
Congrats on setting up the DVLA Vehicle Check bot!

To get details of a vehicle simply send the vehicle registration number or just upload a picture of a vehicle containing the registration number`
    }
  }
